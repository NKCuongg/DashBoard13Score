...
cred = credentials.Certificate(
    'iuh-20007141-firebase-adminsdk-y1n0e-183adc1e34.json')
appLoadData = firebase_admin.initialize_app(cred)

dbFireStore = firestore.client()

queryResults = list(dbFireStore.collection(u'tbl-20007141').stream())
    ...
    children=[
        html.Div(
            children=[
                ...
                html.H4(
                    "Trường: IUH, Môn: Lập Trình Phân Tích Dữ Liệu 1", className="header-title"
                )
            ],
            className="header",
        ),
